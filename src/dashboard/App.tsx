...
function AppInner(): JSX.Element {
  ...
  return (
    <main className="bg-background mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-8 px-6 py-10 sm:px-8">
...
function ShellLoading({ message }: { message: string }): JSX.Element {
  return (
    <div
      className="border-border bg-card text-muted-foreground rounded-lg border px-5 py-8 text-center text-sm"
      role="status"
    >
      <p>{message}</p>
      <p className="mt-2 text-xs">Your history stays on this device.</p>
    </div>
  );
}
...
function ShellEmpty(): JSX.Element {
  return (
    <div className="border-border bg-card rounded-lg border px-5 py-8 text-center">
      <p className="text-foreground font-display text-lg">No browsing rhythm yet</p>
      <p className="text-muted-foreground mt-2 text-sm">
        Visit a few sites over the next days. Cells will fill in here automatically.
      </p>
    </div>
  );
}
